<template>
  <section>
    <div
      class="min-h-screen overflow-hidden p-5 md:p-10 bg-accent-tint pt-20 md:pt-32"
    >
      ...
      <div class="rounded-md border bg-zinc-50 my-5 p-5 md:p-8">
        ...
        <div v-else>
          <div
            class="flex flex-col md:items-center lg:flex-row justify-between"
          >
...
const handleEditQrCode = () => {
  router.replace(`/user/qrcode/edit/${id}`);
};
...
const getSingleQrcode = async () => {
  loading.value = true;
  const result = await $axiosHelper.get(`qr_code/qrcode/${id}`, sessiondata);
  if (!result.error) {
