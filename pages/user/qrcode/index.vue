<template>
  <section
    class="min-h-screen overflow-hidden px-3 md:px-10 bg-white-tint pt-20 md:pt-24"
  >
    <div
      class="flex flex-col md:flex-row items-start md:items-center justify-between"
    >
      <div>
        <HeaderComponent :title="'My QR Codes'" :subTitle="''" />
      </div>
    ...
    <section class="h-[600px] w-full scrollbar-hide overflow-y-auto">
      <div>
        ...
        <div class="">
          ...
          <div v-else v-for="code in generatedCodes" :key="code.id">
            <div
              class="flex flex-col lg:flex-row justify-between my-5 p-2 md:p-8 bg-white shadow-sm rounded-md"
            >
              <div
                class="flex flex-col lg:flex-row items-start lg:items-center gap-1 md:gap-5"
              >
                <!-- <FrameOne :code="code" /> -->
                <!-- <FrameTwo :code="code" /> -->
    ...
    <!-- PAgination -->
    <div v-if="total_items >= 5" class="flex justify-end px-5 mt-2">
...
const queryParams = "?" + new URLSearchParams(queryObject.value).toString();

const getAllQrCode = async () => {
  loading.value = true;

  const result = await $axiosHelper.get(
    `qr_code/qrcode${queryParams}&page=${page.value}&per_page=${per_page.value}`,
    sessiondata
  );
...
const handlehiddenQrcode = async (code: any, type: boolean) => {
  ...
  const result = await $axiosHelper.patch(
    `qr_code/qrcode/${code.id}`,
    {
      hide: type,
    },
    sessiondata
...
const handleQrStylingSave = async (updatedStyles: any) => {
  ...
  const result = await $axiosHelper.post(
    `qr_code/style_qrcode/${updatedStyles.id}`,
