<template>
  <section class="h-screen px-5 md:px-10 pt-20 md:t-24 bg-accent-tint">
    <div class="flex flex-col md:flex-row md:items-center justify-between">
      <div class="flex justify-between md:justify-start items-center gap-5">
        <HeaderComponent :title="'My Links'" :subTitle="''" />
        <NuxtLink to="/user/links/new">
          <button
          ...
          <popper>
            ...
            <template #content>
              <div class="shadow bg-white w-[150px] rounded-md">
                ...
                >
                  <p class="py-2 px-3 font-medium capitalize">{{ status }}</p>
                </div>
              </div>
            </template>
          </popper>
    ...
    <section class="h-[600px] overflow-y-scroll">
      <div
        v-if="loading"
...
const getLinks = async () => {
  ...
  const result = await $axiosHelper.get(
    `url_shortener/short_urls${queryParams}&page=${page.value}&per_page=${per_page.value}`,
    sessiondata
  );
  if (!result.error) {
    loading.value = false;
...
const handleEditLink = async (item: any) => {
  ...
  const response = await $axiosHelper.patch(
    `url_shortener/short_url/${item.id}`,
    {
      title: item.title,
